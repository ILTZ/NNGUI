...
class Ui_FAQDialog(object):
    def setupUi(self, FAQDialog):
        ...
        self.scrollAreaWidgetContents.setGeometry(QtCore.QRect(0, 0, 665, 1019))
...
"получить требуемый результат). Производить стандартный опрос имеет смысл лишь после обучения сети на какой-либо выборке данных."))
        ...
        self.label_6.setText(_translate("FAQDialog", "Данный раздел отвечает за внутренние параметры сети. В левой части экрана пользователь может изменять количество входных/выходных значений, количество нейронов в 1-м и 2-м скрытых слоях (после данных изменений веса должны быть сгенерированы заново, за что отвечает одноименная кнопка). Параметры \"Количество циклов\" и \"Количество эпох\" будут определять, на сколько точно нейросеть будет обучена на представляемой выборке. Параметр \"Коэффициент обучения\" отвечает за то, на сколько \"быстро\" нейросеть будет обучаться (как сильно её веса будут меняться за 1 цикл). Если текущие веса нейросети удовлетворяют целям пользователя, он может их сохранить, нажав на кнопку \"Сохранить веса\". \"Загрузить веса\" отвечает за одноименное действие (перед загрузкой весов необходимо привести нейросеть в то состояние, в котором загружаемые веса сохранялись)."))
